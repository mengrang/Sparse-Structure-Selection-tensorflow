...
def stack_blocks_dense(net,
  ...
  for block in blocks:
    with variable_scope.variable_scope(block.scope, 'block', [net]) as sc:
      ...
      # In that case, if the same name has been previously used in the same scope, 
      # it will be made unique by appending _N to it.
      for i, unit in enumerate(block.args):
        ...
        with variable_scope.variable_scope('unit_%d' % (i + 1), values=[net]):
          ...
          #TODO: lambda_decay
          # lambda_block = tf.Variable(tf.ones(shape=[1, 1, 1, c], dtype=tf.float32), name='lambda_block')
          lambda_block = tf.get_variable(name='lambda_block', shape=[1, 1, 1, 1], dtype=tf.float32, 
                  initializer=tf.ones_initializer(), regularizer=tf.contrib.layers.l1_regularizer(LAMBDA_DECAY))
          tf.add_to_collection('lambda_block', lambda_block)
          # lambda_block = tf.contrib.layers.l1_regularizer(unit_lambda_decay, scope='lambda_block')(lambda_block)
          # lambda_block = utils.collect_named_outputs(outputs_collections, lambda_block.name, lambda_block)
          ...
          if output_stride is not None and current_stride == output_stride:            
            ...
            # net = tf.multiply(tf.tile(lambda_block, [n, h, w, c]), net)
            net = net * lambda_block
